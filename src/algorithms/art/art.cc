  ...
  while (n) {
    ...
    if (IS_LEAF(n)) {
      ...
      if (!leaf_matches((art_leaf*)n, key, key_len, depth)) {
        return reinterpret_cast<uint64_t>(((art_leaf*)n)->value);
      }
      return NULL;
    }
...
static void* recursive_insert(art_node* n, art_node** ref, const unsigned char* key, int key_len,
...
static art_leaf* recursive_delete(art_node* n, art_node** ref, const unsigned char* key,
 ...
 */
int art_iter(art_tree* t, art_callback cb, void* data) { return recursive_iter(t->root, cb, data); }
 ...
 * @return 0 on success, or the return of the callback.
 */
int art_iter_prefix(art_tree* t, const unsigned char* key, int key_len, art_callback cb,
                    void* data) {
  art_node** child;
  art_node* n = t->root;
  int prefix_len, depth = 0;
