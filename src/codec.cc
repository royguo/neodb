...
void Codec::EncodeWriteBuffer(std::unique_ptr<WriteBuffer> buffer, char* buf,
  ...
  // 1. Estimate total buffer size
}

void Codec::DecodeSingleItem(uint64_t offset, std::string* key,
...
uint32_t Codec::EstimateEncodedBufferSize(uint32_t current_sz, uint32_t key_sz,
  ...
  uint32_t padding_sz = current_sz % CRC_BLOCK_SIZE > 0
                            ? CRC_BLOCK_SIZE - (current_sz % CRC_BLOCK_SIZE)
                            : 0;
